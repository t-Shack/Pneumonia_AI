"""
Generates all charts and graphs for the report. Works with any number of
trained models — discovers whichever labels are present rather than
...
All charts are saved as high-DPI PNGs in outputs/charts/.
"""
...
def load_history(label):
    """v2 history files are {"history": {...}, "phase_boundary_epoch": N}
    (warm-up -> fine-tune boundary, for annotating the chart). Falls back to
    treating the file as a plain history dict if it's the older v1 format."""
...
def chart_training_curves():
    ...
    if not labels:
        print("No training history found — skipping training curves chart.")
        return
    ...
    for col, label in enumerate(labels):
        ...
        if phase_boundary:
            ...
            ax_acc.text(phase_boundary + 0.6, ax_acc.get_ylim()[0], "fine-tune\nstarts",
...
def chart_roc_curves(eval_results, filename="03_roc_curves.png", title_suffix=""):
    ...
    ax.set_title(f"ROC Curve{title_suffix} — Pneumonia Detection")
    ax.legend()
    fig.tight_layout()
    _save(fig, filename)
...
def chart_pr_curves(eval_results):
    ...
    ax.set_title("Precision-Recall Curve — Pneumonia Detection")
    ax.legend()
    fig.tight_layout()
...
def chart_model_comparison(eval_results):
    ...
    axes[1].bar(labels, losses_, color=colors)
...
def chart_filter_progression():
    """From the original from-scratch architecture (kept for the paper's
    record of that design) — not meaningful for a pretrained backbone, so
    skipped automatically when config.BACKBONE isn't the old custom CNN."""
...
def _pick_balanced_samples(test_gen, per_class):
    """test_gen has shuffle=False and reads class subfolders in order, so
    the first batch is single-class only — walk batches to get a mix."""
...
def chart_sample_predictions(n=12, model_label="sigmoid"):
    """Uses the RAW [0,1] generator for display (so the images actually
    look like X-rays) and applies backbone preprocessing separately just
    for the model.predict() call — imshow()'ing a DenseNet-preprocessed
    tensor directly would render as meaningless noise, since those values
    sit around [-2, 2.7], not [0, 1]."""
    ...
    for i in range(n):
        ...
        axes[i].set_title(
            f"True: {idx_to_class[true_idx[i]]}\n"
            f"Pred: {idx_to_class[pred_idx]} ({confidence:.2f})",
...
def chart_gradcam(n=8, model_label="sigmoid"):
    ...
    for i in range(n):
        ...
        color = "green" if correct else "red"
        axes[i].set_title(
            f"True: {idx_to_class[true_idx[i]]} | Pred: {idx_to_class[pred_idx]} ({pred[pred_idx]:.2f})",
            color=color, fontsize=8,
        )
    for j in range(n, len(axes)):
        axes[j].axis("off")

    fig.suptitle(f"Grad-CAM — {model_label} (red/yellow = regions driving the prediction)", y=1.02)
    fig.tight_layout()
...
def chart_external_comparison(primary_results, external_results):
    """Side-by-side accuracy: primary (Kermany) test set vs. external (NIH)
    test set, per model — the actual generalization claim."""
    labels = ordered_labels(set(primary_results.keys()) & set(external_results.keys()))
    if not labels:
        print("No overlapping model labels between primary and external results — skipping.")
        return
    ...
    ax.set_ylim(0, 1)
    ax.set_ylabel("Accuracy")
    ax.set_title("Generalization: Primary vs. External Test Set")
    ax.legend()
...
def main():
    ...
    if os.path.exists(external_path):
        ...
        print("Skipping external-dataset charts — run external_dataset_prepare.py "
              "then evaluate_external.py first, if you want these.")
